(function () {
	...
	app
		...
		.service('contentDataService', [
			function () {
				return {
					getBilingualLanguage: function () { return 'en-US'; }
				}
			}
		])
		.service('zoomService', [
			...
			function ($scope, $window, $timeout, constantService, tutorialHelperService, scratchworkTutorialService, themeService) {
				...
				$scope.tutorialToolbarSettings = {
					...
					bilingual: {
						...
						isSpanish: function ($event) { return $scope.tutorial.iconStyles.bilingualIconStyle.getKey() === constantService.LANGUAGETYPES.SPANISH; }
					},
					equationEditor: {
